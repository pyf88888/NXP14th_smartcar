...
【软件版本】V1.0
【最后更新】2017年11月24日 
【函数名】  注意： 电机1： 使用 L5  M5   电机2：使用A8  A9  电机3：使用 B9 C9  电机4：A10 J1
...
short Servo_K_Ctrl(float error)
{
	...
	//增量式pid
	ThisErr = error;
    pid_speed_pErr = ThisErr - pid_speed_LastErr;//比例误差
	pid_speed_iErr = ThisErr;                       //积分误差
...
void Control_Servo_K(float bat,float angle)
{
    ...
    batv0 = (float)(batv0*0.9 + 0.1*(float)(ReadADC(ADC2,ADC2_CH10)*4.8));//电池电压采集 
    if(angle > 30)                       //输入角度限幅
        angle = 30;
    if(angle < -30)
        angle = -30;
    
    angle = (angle + 30)/60 * 0.8 + 2.1;  //将角度映射到反馈 的电压上
    float error = 0;

    error = angle - bat/1000;
    short PWM = Servo_K_Ctrl(error);     //简单的PID控制器
    
    if(PWM < 5000 && PWM > 1500)          //防止转矩太小，不能到达指定位置
    {
        PWM = 5000;
    }
    if(batv0 > 7000)                      //K车舵机最大7.2V  这里用7V
    {
...
void Test_Servo_K(void)
{
    ...
    {    
        ...
        bat_servo = (float)(bat_servo * 0.90 + 0.1 * (float)(ReadADC(ADC2,ADC2_CH9) * 0.806)); //伺服电机反馈电压采集
        
        switch(LQ_KEY_Read(1))  //检测按键
        {
            case 0:
            break;
            case 1:
